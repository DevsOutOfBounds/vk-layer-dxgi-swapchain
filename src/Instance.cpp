...
#define DOOB_print(...) DoobDebugPrint(__VA_ARGS__)
...
#define ASSIGN_DISPATCH(fn) dispatchTable.fn = (PFN_vk##fn)gdpa(*pDevice, "vk"#fn) 
	...
	if (!dispatchTable.GetImageMemoryRequirements2) {
		dispatchTable.GetImageMemoryRequirements2 = ASSIGN_DISPATCH(GetImageMemoryRequirements2KHR);
	}
